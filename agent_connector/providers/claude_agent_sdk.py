...
class ClaudeAgentSdkProvider:
    ...
    def runtime_profile_report(self) -> JsonDict:
        ...
        env_keys = sorted(
            key
            for key in env
            if key != "CLAUDE_AGENT_SDK_CLIENT_APP" and _is_runtime_env_key(key)
        )
    ...
    def _claude_env_overrides(self) -> dict[str, str]:
        ...
        # Claude Agent SDK session resume currently treats ANTHROPIC_API_KEY as
        # the env-auth sentinel. Anthropic-compatible gateways commonly provide
        # ANTHROPIC_AUTH_TOKEN instead, so mirror it for the SDK child process
